...
- <b>번호를 글에 적지 않는다.</b> 목록의 번호(509, 835 …)는 이쪽에서 붙인 것이라
  읽는 사람에게는 아무 뜻이 없다. 「조문 1040은」「351·176·158은」처럼 쓰면 안 된다.
  why·note 에는 반드시 <b>법령 이름과 조 이름</b>으로 적는다
  (「의약품 등의 안전에 관한 규칙 별표 1 8.2 포장공정관리」처럼).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 목록 번호가 글에 새는 꼴이 여럿이다 — [1040] · 제1040조 · 「조문 1040은」 · 「351·176·158은」 · 「519 별표 제6조」.
    // 실제로 note 에 「조문 1040은 일반적인 포장공정 기준이고, 351·176·158은…」이 그대로 나왔다(2026-09-09).
    const lab = (d: string) => index[+d] ? "「" + index[+d].label + "」" : null;
    const nameOf = (t: string) => String(t || "")
      .replace(/\[(\d{1,4})\]/g, (_m, d) => lab(d) || "")
      .replace(/(?:제\s*)?(\d{3,4})(?:\s*[-~–]\s*\d{3,4})?\s*조/g, (m, d) => (+d >= 100 && lab(d)) || m)
      .replace(/조문\s*(\d{1,4})/g, (m, d) => lab(d) || m)
      .replace(/\b(\d{2,4}(?:\s*[·,]\s*\d{2,4})+)\b/g, (m) => m.split(/\s*[·,]\s*/).map((d) => lab(d) || d).join("·"))
      .replace(/\b(\d{2,4})\s+(?=별표|지침|규칙|고시)/g, (m, d) => lab(d) ? lab(d) + " " : m)
      .replace(/\s{2,}/g, " ").trim();
